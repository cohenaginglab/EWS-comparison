...
for (tw in c(2, 3, 4, 12)) {
  ...
  indices <- c("CVPC1", "Df", "MAF_ac", "MAF_ev", "MI", "Av_Ac", "NMA", "MAF_var", "NMV", "Av_Var",
  ...
  dats[[ifelse(tw==2, 1, ifelse(tw==3, 2, ifelse(tw==4, 3, 4)))]] <- dat.all
  rm(list = c(paste("dat", 1:15, sep = ""), "dat", "dat.all"))
  
}

names(dats) <- c("mths2", "mths3", "mths4", "yr1")
saveRDS(dats, "HD indices_diff time windows.rds")
